...
# For students ------------
def train_student(embedding, mlp, mlp_optimizer):
    mlp.train()
...
@torch.no_grad()
def test_student(embedding, mlp):
    mlp.eval()
...
def main():
    ...
    for n in range(times_for_average): 
        ...
        mlp = MLP([hidden, 64, num_classes], dropout=0.5).to(device)
        ...
        for epoch in range(1, 501):
            loss = train_student(embed, mlp, mlp_optimizer)
            if epoch % 20 == 0:
                train_acc, val_acc, test_acc = test_student(embed, mlp)
