...
from logistic_regression import LREvaluator, MLP
from GCL.models.contrast_model import WithinEmbedContrast
from torch_geometric.nn import GCNConv
from torch_geometric.datasets import WikiCS, Planetoid, Amazon, Coauthor, PPI
from ogb.nodeproppred import PygNodePropPredDataset
from pl_bolts.optimizers import LinearWarmupCosineAnnealingLR
from torch.optim.lr_scheduler import SequentialLR, ConstantLR, MultiStepLR
...
class GConv(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.conv1 = GCNConv(input_dim, 2 * hidden_dim, cached=False, dropout=0.1)
        self.conv2 = GCNConv(2 * hidden_dim, hidden_dim, cached=False, dropout=0.1)
...
# For students ------------
def train_student_mlp(y_soft, data, mlp, mlp_optimizer):
    mlp.train()
...
def train_student(y_soft, data, gcnn, gcn_optimizer):
    gcnn.train()
    gcn_optimizer.zero_grad()
    out = gcnn(data.x, data.edge_index)
...
def train_student_via_embed_ssl(embed_ssl, y_soft, data, mlp, mlp_optimizer):
...
def train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, weight_loss1, weight_loss2, weight_loss3):
    ...
    loss1 = F.cross_entropy(out[data.train_mask], data.y[data.train_mask])
    loss2 = 0 #F.kl_div(out.log_softmax(dim=-1), y_soft, reduction='batchmean', log_target=True)
    '''
    if weight_loss3!=0:
        good_nodes_mask = sample_queue_generate(data, 27)
        sample_mask = instance_queue_generate('random', 800, num_data)
        ...
        loss = 0*loss1 + 1*loss2 + weight_loss3*loss3
    else:
        loss = 0*loss1 + 1*loss2
    '''
    if weight_loss3!=0:        
        ls3 = torch.nn.MSELoss()
        loss3 = ls3(embed_mlp, embed_ssl)
        #loss3 = F.kl_div(teacher_prob.log_softmax(dim=-1), student_prob.log_softmax(dim=-1), reduction='batchmean', log_target=True)
        loss = weight_loss3*loss3 #weight_loss1*loss1 + weight_loss2*loss2 + 
    else:
...
@torch.no_grad()
def test_student_mlp(mlp):
    mlp.eval()
    pred = mlp(data.x)
    return pred

@torch.no_grad()
def test_student(mlp2, emb):
    mlp2.eval()
    pred = mlp2(emb).argmax(dim=-1)
    accs = []
...
@torch.no_grad()
def test_student_gcn(gcnn):
    gcnn.eval()
    pred = gcnn(data.x, data.edge_index).argmax(dim=-1)
    accs = []
    for _, mask in data('train_mask', 'val_mask', 'test_mask'):
        accs.append(int((pred[mask] == data.y[mask]).sum()) / int(mask.sum())) 
    return accs
...
def main():
    ...
    if dataset_name=='WikiCS':
        ...
        lrr, hidden, my_pe, my_pf, num_classes = [5e-4, mlp_hidden, 0.0, 0.3, 10]
    elif dataset_name=='Amazon-Photo':
    ...
    for n in range(times_for_average):
        ...
        mlp = MLP(in_channels=data.x.shape[1], hidden_channels = mlp_hidden, out_channels=mlp_hidden, num_layers=3, norm=None, act=torch.nn.PReLU(), dropout=0.0).to(device)
        mlp2 = MLP(in_channels=mlp_hidden, hidden_channels = mlp_hidden, out_channels=num_classes, num_layers=2, dropout=0.0).to(device)
        mlp_optimizer = torch.optim.Adam(mlp.parameters(), lr=0.02, weight_decay=0)
        mlp2_optimizer = torch.optim.Adam(mlp2.parameters(), lr=0.008 , weight_decay=0)
        #gcnn = GCN(in_channels=data.x.shape[1], hidden_channels=mlp_hidden, out_channels=num_classes, num_layers=1, dropout=0.2).to(device)
        #gcn_optimizer = torch.optim.Adam(gcnn.parameters(), lr=0.02, weight_decay=0)
        
        #scheduler1 = LinearWarmupCosineAnnealingLR(optimizer=mlp_optimizer, warmup_epochs=1000, max_epochs=5000)
        #scheduler2 = ConstantLR(optimizer=mlp_optimizer, factor=0.6, total_iters=3000)
        #scheduler3 = ConstantLR(optimizer=mlp_optimizer, factor=0.1, total_iters=3000)
        mlp_scheduler = LinearWarmupCosineAnnealingLR(optimizer=mlp_optimizer, warmup_epochs=1000, max_epochs=5000)
        ...
        embed_ssl = None
        test_result = None
        #with tqdm(total=600, desc='(T)') as pbar:
        ...
        torch.save(embed_ssl, 'tmp.pt')
        embed_ssl = torch.load('tmp.pt')
        embed_ssl = embed_ssl.to(device)
        ...
        for epoch in range(1, 5001):
            ...
            mlp_scheduler.step()
        with torch.no_grad():
            embed_mlp = test_student_mlp(mlp)
            
        print('Loading data')
        torch.save(embed_mlp, 'tmp1.pt')
        embed_mlp = torch.load('tmp1.pt')
        embed_mlp = embed_mlp.to(device)
        
        for epoch in range(1, 3001):
            loss = train_student_via_embed_ssl(embed_mlp, y_soft, data, mlp2, mlp2_optimizer)
            if epoch % 100 == 0:
                train_acc, val_acc, test_acc = test_student(mlp2, embed_mlp)
        ...
        mlp = MLP(in_channels=data.x.shape[1], hidden_channels = mlp_hidden, out_channels=num_classes, num_layers=3, dropout=0.1).to(device)
        mlp_optimizer = torch.optim.Adam(mlp.parameters(), lr=0.01, weight_decay=0)
        
        scheduler1 = LinearWarmupCosineAnnealingLR(optimizer=mlp_optimizer, warmup_epochs=200, max_epochs=200)
        scheduler2 = ConstantLR(optimizer=mlp_optimizer, factor=1, total_iters=1000)
        scheduler3 = ConstantLR(optimizer=mlp_optimizer, factor=0.1, total_iters=300)
        #mlp_scheduler = SequentialLR(optimizer=mlp_optimizer, schedulers=[scheduler1, scheduler2, scheduler3], milestones=[200, 1200])
        ...
        for epoch in range(1, 1501):
            loss = train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, 0.1)
            if epoch % 100 == 0:
                train_acc, val_acc, test_acc = test_student(mlp)
                print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Train: {train_acc:.4f}, '
                      f'Val: {val_acc:.4f}, Test: {test_acc:.4f}')
...
if __name__ == '__main__':
    ...
    os.system('mkdir ./output/GBT/%s/emb'%dataset_name)
    os.system('mkdir ./output/GBT/%s/emb/%d'%(dataset_name, mlp_hidden))
    model_path = './output/GBT/%s/emb/%d/'%(dataset_name, mlp_hidden)
    deg = degree(data.edge_index[0], data.num_nodes).detach().cpu().numpy()
    #a = np.full(data.num_nodes, 1)
