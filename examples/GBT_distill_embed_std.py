...
def train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, weight_loss3):
...
def main():
    ...
    for n in range(times_for_average):
        ...
        gcn_optimizer = torch.optim.Adam(gcnn.parameters(), lr=0.015, weight_decay=0)
        '''
        ...
        for epoch in range(1, 2001):
            '''
            loss = train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, 0)
            if epoch % 100 == 0:
                train_acc, val_acc, test_acc = test_student(mlp)
            '''
            loss = train_student(y_soft, data, gcnn, gcn_optimizer)
            if epoch % 100 == 0:
                train_acc, val_acc, test_acc = test_student(gcnn)
