...
def test(encoder_model, data):
    ...
    result, y_soft, emb = LREvaluator().evaluate(z, data.y, split, model_path)
    return result, y_soft, z, emb
...
def train_student_via_small_emb(embed_small, y_soft, data, mlp, mlp_optimizer):
...
def main():
    ...
    for n in range(times_for_average): 
        ...
        test_stud = 0
        test_emb_stud = 0   
        mlp = MLP([data.x.shape[1], 256, 64, num_classes], dropout=0.0, norm=None, act=torch.nn.PReLU()).to(device)
        ...
        scheduler1 = LinearWarmupCosineAnnealingLR(optimizer=mlp_optimizer, warmup_epochs=1000, max_epochs=7000)
        scheduler2 = ConstantLR(optimizer=mlp_optimizer, factor=0.5, total_iters=3000)
        mlp_scheduler = SequentialLR(optimizer=mlp_optimizer, schedulers=[scheduler1, scheduler2], milestones=[7000])
        ...
        for epoch in range(1, 1001):
            ...
            if epoch % log_interval==0:
                accs, y_soft, embed, embed_small = test(encoder_model, data)
                encoder_model.train()
        ...
        test_result, y_soft, embed_ssl, embed_small = test(encoder_model, data)
        print('check if with grad')
        print(y_soft.requires_grad, '   ', embed_ssl.requires_grad, '   ', embed_small.requires_grad)
        ...
        for epoch in range(1, 7001):
            loss = train_student_via_small_emb(embed_small, y_soft, data, mlp, mlp_optimizer)
            if epoch % 100 == 0:
                print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}')
            mlp_scheduler.step()
        for epoch in range(7001, 10001):
