...
parser.add_argument('--degree',type=int, default=0,help=' Good Nodes degree minimum')
...
def train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, weight_loss3, threshold, num_random):
    ...
    if weight_loss3!=0:
        good_nodes_mask = sample_queue_generate(data, threshold)
        sample_mask = instance_queue_generate('random', num_random, num_data)
...
def main():
    ...
    if dataset_name=='WikiCS':
        ...
        lrr=1e-3
        num_classes=47
    os.system('mkdir output/distill_hyper')
    os.system('mkdir output/distill_hyper/%d' %args.degree)
    log_file = 'output/distill_hyper/%d/log.txt'%args.degree
    os.system('touch %s'%log_file)
    with open(log_file, 'a') as f:
        f.write(dataset_name+'\n')
        f.write('training finished for: %d times\n'%times_for_average)
        f.write('MLP hidden= %d \n'%mlp_hidden)
    for th in range(3):
        threshold = 1 + th*2 + 6*args.degree
        a = np.full(data.num_nodes, 1)
        print(a[deg>=threshold].sum())
        for num_random in [50, 200, 800, 2000, 5000]:
            acc_teacher = []
            acc_student = []
            std_teacher = 0
            std_student = 0
            with open(log_file, 'a') as f:
                f.write('threshold=%d, num_random=%d \n'%(threshold, num_random))
                for n in range(times_for_average): 
                    ...
                    mlp_optimizer = torch.optim.Adam(mlp.parameters(), lr=0.05, weight_decay=0)        
                    ...
                    for epoch in range(1, 5001):
                        loss = train_student_via_embedding(embed_ssl, y_soft, data, mlp, mlp_optimizer, 0.1, threshold, num_random)
                        if epoch % 100 == 0:
                ...
                f.write('average teacher acc: %.6f , and std: %.6f \n'%(teacher_average, std_teacher))
                f.write('average student acc: %.6f , and std: %.6f \n'%(student_average, std_student))
...
if __name__ == '__main__':
    ...
    os.system('mkdir ./output/GBT/%s/hyper'%dataset_name)
    os.system('mkdir ./output/GBT/%s/hyper/%d' %(dataset_name, args.degree))
    model_path = './output/GBT/%s/hyper/%d/'%(dataset_name, args.degree)
