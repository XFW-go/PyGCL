...
# For students ------------
def train_student(embedding, mlp, mlp_optimizer, split_n):
    mlp.train()
    mlp_optimizer.zero_grad()
    out = mlp(embedding)

    loss1 = F.cross_entropy(out[data.train_mask[:,split_n]], data.y[data.train_mask[:,split_n]])
    #loss2 = F.kl_div(out.log_softmax(dim=-1), y_soft, reduction='batchmean', log_target=True)
    loss = 1 * loss1 # + (1 - args.lamb) * loss2
...
@torch.no_grad()
def test_student(embedding, mlp, split_n):
    mlp.eval()
    pred = mlp(embedding).argmax(dim=-1)
    accs = []
    flag = 0
...
def main():
    ...
    for n in range(times_for_average): 
        ...
        gconv = GConv(input_dim=dataset.num_features, hidden_dim=hidden).to(device)
        ...
        for split_n in range(20):                          
            mlp = MLP([hidden, 64, dataset.num_classes], dropout=0.5).to(device)
            mlp_optimizer = torch.optim.Adam(mlp.parameters(), lr=0.01, weight_decay=5e-4)
            
            encoder_model.eval()
            embed, _, _ = encoder_model(data.x, data.edge_index, data.edge_attr)
            ...
            for epoch in range(1, 501):
                loss = train_student(embed, mlp, mlp_optimizer, split_n)
                if epoch % 20 == 0:
                    train_acc, val_acc, test_acc = test_student(embed, mlp, split_n)
