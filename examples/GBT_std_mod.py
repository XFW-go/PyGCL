...
#from tasks import evaluate_node_classification
...
class Encoder(torch.nn.Module):
    ...
    def predict(self, x, edge_index, edge_weight=None):
        self.encoder.eval()
        with torch.no_grad():
            z = self.encoder(x.to(device), edge_index.to(device))
            return z.cpu()
...
def main():
    ...
    if dataset_name=='WikiCS':
        ...
        lrr, hidden, my_pe, my_pf, num_classes = [1e-3, 256, 0.3, 0.1, 47]
    ...
    for n in range(times_for_average): 
        ...
        for epoch in range(1, 1001):
            ...
            if epoch % log_interval==0:
                accs, y_soft = test(encoder_model, data)
                encoder_model.train()
                if accs['acc_val'] > best_val_acc_GBT:
                    best_val_acc_GBT = accs['acc_val']
                    best_test_acc_GBT = accs['acc']
                    # Save the models
        ...
        # Load the best parameters of the GBT and the linear classifier
        ...
        for epoch in range(1, 5001):
            loss = train_student(best_y_soft, data, mlp, mlp_optimizer)
            if epoch % 100 == 0:
    ...
    print(dataset_name)
    print('Layer=3   MLP hidden=%d'% mlp_hidden)
...
if __name__ == '__main__':
    ...
    os.system('mkdir ./output/GBT/%s/0928/%d'%(dataset_name, mlp_hidden))
    model_path = './output/GBT/%s/0928/%d/'%(dataset_name, mlp_hidden)
